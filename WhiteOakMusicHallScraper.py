...
concerts = soup.find_all('div', class_='tw-section')
...
for i, concert in enumerate(concerts, start=2):
    day = concert.find(class_="tw-event-day").text.strip()
    month = concert.find(class_="tw-event-month").text.strip()
    date = concert.find(class_="tw-event-date").text.strip()
    full_date = day + " " + month + " " + date 
    event_name = concert.find(class_="tw-name").text.strip()
    venue = "White Oak Music Hall " + concert.find(class_="tw-venue-name").text.strip()
...
wb.save(f'White Oak Music Hall Concerts_{timestamp}.xlsx')
